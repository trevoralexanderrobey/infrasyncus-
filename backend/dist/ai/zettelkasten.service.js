...
let ZettelkastenService = class ZettelkastenService {
    constructor(prisma, ollamaService, webSearchService) {
        this.prisma = prisma;
    ...
    async searchAndCreateNotes(query, context) {
        try {
            const searchResults = await this.webSearchService.searchAndEnhance(query, context);
            const createdNotes = [];
            if (searchResults.synthesis) {
                const synthesisNote = await this.createAtomicNote(`Web Search Synthesis: ${query}\n\n${searchResults.synthesis}`, [
                    "web-search",
                    "synthesis",
    ...
    async enrichExistingNote(noteId, domain) {
        try {
            ...
            const enrichmentResults = await this.webSearchService.enrichNote(originalNote.content, domain);
            const connections = await this.findPotentialConnections(enrichmentResults.relatedConcepts);
    ...
    async fillKnowledgeGapsWithSearch(gapDescription) {
        try {
            const notes = await this.getAllNotes();
            const existingConcepts = notes
                .map((note) => note.content.substring(0, 50))
                .slice(0, 15);
            const searchResults = await this.webSearchService.fillKnowledgeGap(gapDescription, existingConcepts);
            const suggestedConnections = await this.findPotentialConnections(searchResults.relatedConcepts);
    ...
    async findCurrentInformation(concept, timeframe = "recent") {
        try {
            return await this.webSearchService.findCurrentInformation(concept, timeframe);
        }
        catch (error) {
    ...
    async findPotentialConnections(concepts) {
        const connections = [];
    ...
    calculateCentrality(word, connections) {
        ...
        const incoming = Array.from(connections.values()).reduce((count, targets) => count + (targets.has(word) ? 1 : 0), 0);
    ...
    identifyContentGaps(nodes, edges, communities) {
        ...
        const smallCommunities = Array.from(communityGroups.entries()).filter(([_, size]) => size < 3);
