...
export class ZettelkastenService {
  ...
  async analyzeTextNetwork(text: string): Promise<TextNetworkAnalysis> {
    ...
    const metrics: GraphMetrics = {
      modularity: 0.3,
      density:
        edges.length / Math.max(1, (nodes.length * (nodes.length - 1)) / 2),
      averageClustering: 0,
      averagePathLength: 0,
      diameter: 0,
  ...
  async analyzeImageWithAI(imageBase64: string, query?: string): Promise<any> {
    ...
    const combinedText = `${analysis.textContent} ${analysis.concepts.join(
      " "
    )} ${analysis.reasoning}`;
  ...
  async generateConceptSuggestions(domain?: string): Promise<any> {
    ...
    const existingConcepts = notes
      .map((note) => note.content.substring(0, 100).replace(/[^\w\s]/g, " "))
      .slice(0, 20); // Limit for prompt size

    const suggestions = await this.ollamaService.generateConceptSuggestions(
      existingConcepts,
      domain
    );
  ...
  async generateKnowledgeInsights(): Promise<any> {
    ...
    const aiInsights = await this.ollamaService.generateKnowledgeInsights(
      concepts,
      relationships
    );
    ...
    const conceptSuggestions =
      await this.ollamaService.generateConceptSuggestions(
        concepts.slice(0, 10),
  ...
  async searchAndCreateNotes(
    query: string,
    context?: string
  ): Promise<{ searchResults: EnhancedSearchResponse; createdNotes: any[] }> {
    try {
      // Perform web search
      const searchResults = await this.webSearchService.searchAndEnhance(
        query,
        context
      );

      // Create atomic notes from search results
      const createdNotes: any[] = [];

      // Create a synthesis note
      if (searchResults.synthesis) {
        const synthesisNote = await this.createAtomicNote(
          `Web Search Synthesis: ${query}\n\n${searchResults.synthesis}`,
          [
            "web-search",
            "synthesis",
            ...searchResults.relatedConcepts.slice(0, 3),
          ]
        );
        createdNotes.push(synthesisNote);
      }

      // Create notes from key insights
      for (const insight of searchResults.keyInsights.slice(0, 3)) {
        const insightNote = await this.createAtomicNote(`Insight: ${insight}`, [
          "web-search",
          "insight",
          query.split(" ")[0],
        ]);
        createdNotes.push(insightNote);
      }

      // Create notes from suggested atomic notes
      for (const suggestion of searchResults.suggestedNotes.slice(0, 2)) {
        const suggestionNote = await this.createAtomicNote(suggestion, [
          "web-search",
          "research",
          query.split(" ")[0],
        ]);
        createdNotes.push(suggestionNote);
      }

      return { searchResults, createdNotes };
    } catch (error) {
      console.error("Search and note creation failed:", error);
      return {
        searchResults: this.webSearchService["getFallbackResponse"](query),
        createdNotes: [],
      };
    }
  }
  ...
  async enrichExistingNote(
    ...
    try {
      ...
      // Perform enrichment search
      const enrichmentResults = await this.webSearchService.enrichNote(
        originalNote.content,
        domain
      );

      // Find potential connections with existing notes
      const connections = await this.findPotentialConnections(
        enrichmentResults.relatedConcepts
      );

      return { originalNote, enrichmentResults, connections };
    } catch (error) {
  ...
  async fillKnowledgeGapsWithSearch(gapDescription: string): Promise<{
    searchResults: EnhancedSearchResponse;
    suggestedConnections: any[];
  }> {
    try {
      // Get existing concepts from the knowledge base
      const notes = await this.getAllNotes();
      const existingConcepts = notes
        .map((note) => note.content.substring(0, 50))
        .slice(0, 15); // Limit for performance

      // Perform gap-filling search
      const searchResults = await this.webSearchService.fillKnowledgeGap(
        gapDescription,
        existingConcepts
      );

      // Suggest connections based on search results
      const suggestedConnections = await this.findPotentialConnections(
        searchResults.relatedConcepts
      );

      return { searchResults, suggestedConnections };
    } catch (error) {
      console.error("Knowledge gap filling failed:", error);
      throw error;
    }
  }
  ...
  async findCurrentInformation(
    ...
    try {
      return await this.webSearchService.findCurrentInformation(
        concept,
        timeframe
      );
    } catch (error) {
      console.error("Current information search failed:", error);
      throw error;
    }
  }

  private async findPotentialConnections(concepts: string[]): Promise<any[]> {
    const connections: any[] = [];

    for (const concept of concepts.slice(0, 5)) {
      // Limit to top 5 concepts
      const relatedNotes = await this.searchGraphNodes(concept);
      if (relatedNotes.length > 0) {
        connections.push({
          concept,
          relatedNotes: relatedNotes.slice(0, 3), // Top 3 related notes
          connectionStrength: relatedNotes[0]?.relevance || 0.5,
        });
      }
    }
    ...
    if (fileName.endsWith(".txt") || fileName.endsWith(".md")) {
      ...
      for (const paragraph of paragraphs) {
        if (paragraph.trim().length > 10) {
          const note = await this.createAtomicNote(paragraph.trim(), [
            `imported:${fileName}`,
          ]);
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (values[0] && values[0].trim().length > 0) {
          const note = await this.createAtomicNote(values[0].trim(), [
            `imported:${fileName}`,
          ]);
          importedNotes.push(note);
        }
  ...
  async getGraphStats(): Promise<any> {
    ...
    return {
      ...
      density:
        links.length / Math.max(1, (notes.length * (notes.length - 1)) / 2),
  ...
  async searchGraphNodes(query: string): Promise<any[]> {
    ...
    return notes.map((note) => ({
      id: note.id,
      content: note.content,
      relevance: note.content.toLowerCase().includes(query.toLowerCase())
        ? 1
        : 0.5,
    }));
  }
  ...
  async getConceptCentrality(): Promise<any[]> {
    ...
    links.forEach((link) => {
      centrality.set(
        link.sourceNoteId,
        (centrality.get(link.sourceNoteId) || 0) + 1
      );
      centrality.set(
        link.targetNoteId,
        (centrality.get(link.targetNoteId) || 0) + 1
      );
    });
  ...
  private buildNetwork(
    ...
    connections.forEach((targets, source) => {
      targets.forEach((weight, target) => {
        if (
          wordFrequency.has(source) &&
          wordFrequency.has(target) &&
          weight > 1
        ) {
          edges.push({
            source,
  ...
  private calculateCentrality(
    ...
    const incoming = Array.from(connections.values()).reduce(
      (count, targets) => count + (targets.has(word) ? 1 : 0),
      0
    );
  ...
  private identifyContentGaps(
    ...
    communities.forEach((communityId) => {
      communityGroups.set(
        communityId,
        (communityGroups.get(communityId) || 0) + 1
      );
    });

    const smallCommunities = Array.from(communityGroups.entries()).filter(
      ([_, size]) => size < 3
    );
  ...
  private async generateInsights(
    ...
    const topTopics = topics
      .slice(0, 3)
      .map((topic) => topic.slice(0, 3).join(", "));
