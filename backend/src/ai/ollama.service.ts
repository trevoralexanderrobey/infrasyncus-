...
export class OllamaService {
  ...
  private availableModels: ModelConfig[] = [
    {
      ...
      ramUsage: 1.9,
      description:
        "Qwen 2.5 3B - Efficient text model with tool support (WEB SEARCH)",
      available: false,
    },
    {
      name: "codellama:7b",
  ...
  getRecommendedModels(): { text: string; multimodal: string } {
    const multimodalModels = this.availableModels.filter(
      (m) => m.type === "multimodal" && m.available
    );
    const codeModels = this.availableModels.filter(
      (m) => m.type === "code" && m.available
    );

    return {
      text:
        codeModels.length > 0 ? codeModels[0].name : "No code model available",
      multimodal:
  ...
  async analyzeCode(
    code: string,
    language?: string
  ): Promise<{ concepts: string[]; explanation: string; complexity: string }> {
    ...
    if (!codeModel) {
      return {
        concepts: ["programming", "code", language || "unknown"].filter(
          Boolean
        ),
...
COMPLEXITY: Simple|Medium|Complex`;
  ...
  async analyzeImage(
    ...
    // Use getAvailableModels() to ensure we're checking the latest available models
    const availableModels = this.getAvailableModels();
    const multimodalModel = availableModels.find(
      (m) => m.type === "multimodal"
    );
...
REASONING: What this image is about`;
    ...
    try {
      ...
      return {
        ...
        concepts: this.extractFromResponse(responseText, "CONCEPTS:")
          ...
          .map((c) => c.trim())
          .filter((c) => c.length > 0),
        codeSnippets: [this.extractFromResponse(responseText, "CODE:")].filter(
          (c) => c.length > 0
        ),
        diagrams: [this.extractFromResponse(responseText, "DIAGRAMS:")].filter(
          (d) => d.length > 0
        ),
  ...
  async generateConceptSuggestions(
...
Respond with bullet points.`;
  ...
  private extractFromResponse(response: string, prefix: string): string {
    ...
    return targetLine
      ? targetLine.replace(new RegExp(prefix, "i"), "").trim()
      : "";
  }
