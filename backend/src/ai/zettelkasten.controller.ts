...
export class ZettelkastenController {
  ...
  async analyzeTextIncremental(
    ...
    return this.zettelkastenService.analyzeTextIncremental(
      text,
      previousAnalysis
    );
  }
  ...
  async getConceptNeighborhood(
    ...
    return this.zettelkastenService.getConceptNeighborhood(
      concept,
      parseInt(depth) || 2
    );
  }
  ...
  async findSimilarConcepts(
    ...
    return this.zettelkastenService.findSimilarConcepts(
      concept,
      parseInt(limit) || 5
    );
  }
  ...
  @Post("search/fill-gaps")
  async fillKnowledgeGaps(
    @Body("gapDescription") gapDescription: string,
    @Body("password") password: string
  ) {
    if (password !== process.env.ZETTELKASTEN_PASSWORD) {
      throw new UnauthorizedException("Invalid password");
  ...
  @Get("search/current-info")
  async getCurrentInformation(
    @Query("concept") concept: string,
    @Query("timeframe") timeframe: "recent" | "latest",
    @Query("password") password: string
  ) {
    if (password !== process.env.ZETTELKASTEN_PASSWORD) {
      throw new UnauthorizedException("Invalid password");
...
<gexf xmlns="http://www.gexf.net/1.2draft" version="1.2">
  ...
  <edges>`;
    ...
    analysis.edges.forEach((edge: any, index: number) => {
      gexf += `
    <edge id="${index}" source="${edge.source}" target="${
        edge.target
  ...
  private generateCSV(analysis: TextNetworkAnalysis): {
    nodes: string;
    edges: string;
  } {
    // Generate CSV format
    ...
    analysis.nodes.forEach((node: any) => {
      nodesCsv += `"${node.id}","${node.label}",${node.size || 5},"${
        node.color || "#95a5a6"
      }",${node.community || 0},${node.betweenness || 0}\n`;
    });

    let edgesCsv = "source,target,weight,label\n";
    analysis.edges.forEach((edge: any) => {
      edgesCsv += `"${edge.source}","${edge.target}",${edge.weight || 1},"${
        edge.label || ""
      }"\n`;
    });
